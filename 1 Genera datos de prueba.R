...
generate_synthetic_data <- function() {
  ...
  r <- rast(nrows=100, ncols=100, xmin=0, xmax=1000, ymin=0, ymax=1000, 
            crs="EPSG:32614") # UTM zona 14N como ejemplo (unidades métricas)
  ...
  elasticidades <- data.frame(
    ...
    rampa = c(0.16, 0.16, 0.16) # 16% cada 100m
  )
  write_csv(elasticidades, "elasticidades.csv")
  
  # 1.3 Crear un polígono de intervención (un parque en el centro)
  # Creamos un cuadrado de 200x200m en el centro
  ...
  # 1.4 Crear CSV de mapeo de intervenciones
  mapeo <- data.frame(
